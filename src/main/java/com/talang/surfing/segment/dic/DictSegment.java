...
class DictSegment implements Comparable<DictSegment> {
    ...
    public Set<DictType> dictTypes = new HashSet<>();
    ...
    public Set<String> props = Sets.newHashSet();
        ...
        if (ds != null) {
            if (length > 1) {
                ...
                if (ds.nodeState == 1) {
                    //添加HIT状态为完全匹配
                    searchHit.setMatch();
                    searchHit.setDictTypes(ds.dictTypes);
     ...
     * @param charArray
     */
    void fillSegment(char[] charArray, DictType dictType) {
        if(charArray.length == 0 ){
            return;
        }
        this.fillSegment(charArray, 0, charArray.length, 1, dictType, "");
    }
     ...
     * @param charArray
     */
    void fillSegment(char[] charArray, DictType dictType, String prop) {
        this.fillSegment(charArray, 0, charArray.length, 1, dictType, prop);
    }
     ...
     * @param charArray
     */
    void disableSegment(char[] charArray, DictType dictType) {
        this.fillSegment(charArray, 0, charArray.length, 0, dictType, "");
    }
     ...
     * @param enabled
     */
    private synchronized void fillSegment(char[] charArray, int begin, int length, int enabled, DictType dictType, String prop) {
        char currentChar = CharacterUtil.regularize(charArray[begin]);
        //获取字典表中的汉字对象
        Character beginChar = new Character(currentChar);
        ...
        if (ds != null) {
            ...
            if (length > 1) {
                //词元还没有完全加入词典树
                ds.fillSegment(charArray, begin + 1, length - 1, enabled, dictType, prop);
            } else if (length == 1) {
